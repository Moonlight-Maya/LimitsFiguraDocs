...
local function myFunction(isToggled)
...
local function toggleCube(isToggled)
	models.die.cube:setVisible(isToggled)
end

local function changeCubeSpeed(scrollAmount)
...
Next up, we'll add a bit more excitement to our cube, making it [play a sound and emit particles](p6_sound_particles.md) when we change its visibility.
